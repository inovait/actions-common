...
function fetchGithubUsernames(options, customFetch, sortedCommits, githubUsernameMap) {
    var _a, _b, _c, _d;
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const commit of parsedCommits) {
            ...
            if (((_a = options.excludeEmails) === null || _a === void 0 ? void 0 : _a.includes(email)) === true) {
                continue;
            }
            console.log(email);
            if (!githubUsernameMap.has(email)) {
                try {
                    ...
                    const res = yield octokit.request(`GET /search/commits?q=${query}`, {
                        per_page: 1,
                        sort: 'committer-date',
                        order: 'desc'
                    });
                    console.log(res);
                    const username = (_d = (_c = (_b = res.data.items) === null || _b === void 0 ? void 0 : _b.at(0)) === null || _c === void 0 ? void 0 : _c.author) === null || _d === void 0 ? void 0 : _d.login;
                    if (username != null) {
...
function generateChangelog(commits, options, customFetch = undefined) {
    return __awaiter(this, void 0, void 0, function* () {
        const sortedCommits = commits.sort((a, b) => a.date < b.date ? -1 : 1);
        const parsedCommits = (0, commit_parsing_1.parseCommits)(sortedCommits);
        const githubUsernameMap = new Map();
        if (options.githubToken != null && options.githubToken.trim() !== '' && options.showNames === true) {
            yield fetchGithubUsernames(options, customFetch, sortedCommits, githubUsernameMap);
        }
        let changelog = '';
        changelog += generateCommitCategory(parsedCommits.filter((commit) => (commit.breaking === true) && commit.type === 'feat'), 'BREAKING Features', options, githubUsernameMap);
        changelog += generateCommitCategory(parsedCommits.filter((commit) => (commit.breaking === true) && commit.type === 'fix'), 'BREAKING Bug Fixes', options, githubUsernameMap);
        changelog += generateCommitCategory(parsedCommits.filter((commit) => (commit.breaking === false) && commit.type === 'feat'), 'Features', options, githubUsernameMap);
        changelog += generateCommitCategory(parsedCommits.filter((commit) => (commit.breaking === false) && commit.type === 'fix'), 'Bug Fixes', options, githubUsernameMap);
        changelog += generateCommitCategory(parsedCommits.filter((commit) => (commit.type != null) && commit.type !== 'feat' && commit.type !== 'fix' && ((commit.breaking === true) || commit.jiraTicket)), 'Miscellaneous', options, githubUsernameMap);
        changelog += generateCommitCategory(parsedCommits.filter((commit) => commit.type == null), '', options, githubUsernameMap);
        return changelog.trim();
    });
}
exports.generateChangelog = generateChangelog;
function generateCommitCategory(commits, categoryTitle, options, githubUsernameMap) {
    if (commits.length === 0) {
        return '';
    }
    let categoryText = '';
...
function generateCommitList(commits, options, githubUsernameMap) {
    var _a;
    let listText = '';
    for (const commit of commits) {
        ...
        if (options.showNames === true && ((_a = options.excludeEmails) === null || _a === void 0 ? void 0 : _a.includes(authorEmail)) !== true) {
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const showNames = core.getInput('show_names') === 'true';
            const excludeEmails = core.getMultilineInput('excluded_name_emails');
            const githubToken = core.getInput('github_token');
            const commits = (0, commit_gathering_1.gatherCommits)('.', fromCommit, toCommit);
            const changelog = yield (0, generator_1.generateChangelog)(commits, {
                gitCommitUrlPrefix,
                jiraUrl,
                showNames,
                excludeEmails,
