...
test("generate changelog for commits without semantic data", async() => {
  const commits =
    [
      createFakeCommit(
  ...
  const actual = await generateChangelog(commits, {
...
test("generate changelog for a minor release", async() => {
  const commits =
    [createFakeCommit(
      "abd0ae703769b889814fc1c896b55e96a1e9dbce",
...
test("generate changelog for a major release", async() => {
  const commits =
    [createFakeCommit(
      "abd0ae703769b889814fc1c896b55e96a1e9dbce",
      "fix: lock OnDemandProvider into mutex",
...
test("generate changelog for a major release with exclamation syntax", async() => {
  const commits =
    [createFakeCommit(
      "abd0ae703769b889814fc1c896b55e96a1e9dbce",
      "fix: lock OnDemandProvider into mutex",
...
test("hide Jira tickets if Jira URL is not provided", async() => {
  const commits =
    [createFakeCommit(
      "abd0ae703769b889814fc1c896b55e96a1e9dbce",
      "fix: lock OnDemandProvider into mutex",
...
test("show Jira tickets and links if Jira URL is provided", async() => {
  const commits =
    [createFakeCommit(
      "abd0ae703769b889814fc1c896b55e96a1e9dbce",
      "fix: lock OnDemandProvider into mutex",
...
test("show Github tickets and links if Jira (interpted as Github) URL is provided", async() => {
  const commits =
    [createFakeCommit(
      "abd0ae703769b889814fc1c896b55e96a1e9dbce",
      "fix: lock OnDemandProvider into mutex",
...
test("Show unimportant commits if they have Jira tickets assigned", async() => {
  const commits =
    [createFakeCommit(
      "abd0ae703769b889814fc1c896b55e96a1e9dbce",
      "fix: lock OnDemandProvider into mutex",
...
test("generate changelog for a patch release", async() => {
  const commits =
    [createFakeCommit(
      "abd0ae703769b889814fc1c896b55e96a1e9dbce",
      "fix: lock OnDemandProvider into mutex",
...
test("Show commit author name when github token is not provided", async() => {
  const commits = [
    createFakeCommit(
      "abd0ae703769b889814fc1c896b55e96a1e9dbce",
      "fix: lock OnDemandProvider into mutex",
    ...
    createFakeCommit(
      ...
      1,
      "Katrina Buckley",
      "katrina.buckley@inova.si"
    )
  ];

  const expected = fs
    .readFileSync(
      "test/test-files/example-minor-release-changelog-with-author-names.txt"
    )
    .toString()
    .trim();
  const actual = await generateChangelog(commits, {
...
test("Show commit github id", async() => {
  const mockFetch = async(urlString: string) => {
    console.log(urlString)
    const url = new URL(urlString);

    if (url.pathname == "/search/commits") {
      if (url.searchParams.get("q") === "author-email:\"john.smith@inova.si\"") {
          return {
            ok: true,
            status: 200,
            headers: new Headers(),
      ...
      if (
        url.searchParams.get("q") === "author-email:\"katrina.buckley@inova.si\""
      ) {
        return {
          ok: true,
          status: 200,
  ...
  const expected = fs
    .readFileSync(
      "test/test-files/example-minor-release-changelog-with-github-mentions.txt"
    )
    .toString()
    .trim();
  const actual = await generateChangelog(commits, {
...
test("Fallback to names when github search returns no results", async() => {
  const mockFetch = async(urlString: string) => {
    const url = new URL(urlString);
  ...
  const expected = fs
    .readFileSync(
      "test/test-files/example-minor-release-changelog-with-mixed-names.txt"
    )
    .toString()
    .trim();
  const actual = await generateChangelog(commits, {
...
test("Fallback to names when github search fails", async() => {
  const mockFetch = async(urlString: string) => {
    const url = new URL(urlString);
...
test("Exclude configured names from mentioning", async () => {
  const mockFetch = async (urlString: string) => {
    const url = new URL(urlString);
  ...
  const expected = fs
    .readFileSync(
      "test/test-files/example-minor-release-changelog-with-excluded-names.txt"
    )
    .toString()
    .trim();
  const actual = await generateChangelog(
    commits,
    {
      gitCommitUrlPrefix: "http://hydra/testProject/commit/",
      showNames: true,
