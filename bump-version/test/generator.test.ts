...
import { getVersionToBump } from '../src/automatic_version_detector'

test('detect minor release', () => {
  const commits =
    [createFakeCommit(
  ...
  expect(getVersionToBump(commits)).toBe('minor')
})

test('detect major release', () => {
  const commits =
    [createFakeCommit(
      'abd0ae703769b889814fc1c896b55e96a1e9dbce',
      'fix: lock OnDemandProvider into mutex',
  ...
  expect(getVersionToBump(commits)).toBe('major')
})

test('detect major release with exclamation syntax', () => {
  const commits =
    [createFakeCommit(
      'abd0ae703769b889814fc1c896b55e96a1e9dbce',
      'fix: lock OnDemandProvider into mutex',
...
test('detect patch release', () => {
  ...
  expect(getVersionToBump(commits)).toBe('patch')
})

test('fallback to patch release for commits without semantic data', () => {
  const commits =
    [
      createFakeCommit(
        'abd0ae703769b889814fc1c896b55e96a1e9dbce',
...
test('detect no release', () => {
  ...
  expect(getVersionToBump(commits)).toBe('none')
})
